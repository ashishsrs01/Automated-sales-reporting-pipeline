...
def generate_recommendations(dataframe, monthly_metrics, insights):
    recommendations = []
    if insights.largest_decline_rate is not None and insights.largest_decline_rate < -10:
        recommendations.append(Recommendation(title='Investigate revenue decline', description=f'Revenue declined by {abs(insights.largest_decline_rate):.1f}% in {insights.largest_decline_month} compared with the previous month.', severity='warning'))
...
def _regional_concentration(dataframe):
    ...
    if share <= 50:
        return None
    return Recommendation(title='Monitor regional concentration', description=f'{top_region} contributes {share:.1f}% of total revenue. Consider monitoring dependence on this region.', severity='warning')
...
def _category_concentration(dataframe):
    ...
    if share <= 60:
        return None
    return Recommendation(title='Monitor category concentration', description=f'{top_category} contributes {share:.1f}% of total revenue. Consider monitoring dependence on this category.', severity='warning')