...
def generate_recommendations(
        ...
        recommendations.append(
            Recommendation(
                ...
                description=(
                    ...
                    f"{abs(insights.largest_decline_rate):.1f}% "
                    f"in {insights.largest_decline_month} "
    ...
    regional_recommendation = _regional_concentration(
        dataframe
    )
...
def _regional_concentration(
    ...
    """Detect excessive revenue concentration in one region."""
    ...
    regional_revenue = (
        dataframe.groupby("Region")["Revenue"]
        .sum()
    )
    ...
    share = (
        regional_revenue.loc[top_region]
        / total_revenue
        * 100
    )
    ...
    return Recommendation(
        ...
        description=(
            f"{top_region} contributes "
            f"{share:.1f}% of total revenue. "
            "Consider monitoring dependence on this region."
        ),
        severity="warning",
    )
...
def _category_concentration(
    ...
    """Detect excessive revenue concentration in one category."""
    ...
    share = (
        category_revenue.loc[top_category]
        / total_revenue
        * 100
    )
