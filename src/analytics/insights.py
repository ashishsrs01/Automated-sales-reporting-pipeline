...
class BusinessInsights:
    ...
    def __init__(self, top_region, top_category, top_product, top_salesperson, best_month, worst_month, strongest_growth_month, strongest_growth_rate, largest_decline_month, largest_decline_rate):
...
def _top_dimension(dataframe, dimension):
    ...
    grouped = dataframe.groupby(dimension, dropna=False)['Revenue'].sum().sort_values(ascending=False)
    if grouped.empty:
        return None
...
def generate_business_insights(dataframe, monthly_dataframe):
    ...
    return BusinessInsights(top_region=top_region(dataframe), top_category=top_category(dataframe), top_product=top_product(dataframe), top_salesperson=top_salesperson(dataframe), best_month=best_revenue_month(monthly_dataframe), worst_month=worst_revenue_month(monthly_dataframe), strongest_growth_month=growth_month, strongest_growth_rate=growth_rate, largest_decline_month=decline_month, largest_decline_rate=decline_rate)