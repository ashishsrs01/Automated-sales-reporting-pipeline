...
def calculate_monthly_metrics(
    ...
    """Calculate monthly revenue, orders, and units."""
    required_columns = {
        "Order_Date",
        "Order_ID",
        "Quantity",
    ...
    if result["Order_Date"].isna().any():
        raise ValueError(
            "Order_Date contains invalid or missing values."
        )
...
def calculate_mom_growth(
    ...
    result = result.sort_values("Month").reset_index(
        drop=True
    )
