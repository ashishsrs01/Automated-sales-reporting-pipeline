...
def calculate_overall_metrics(dataframe):
    ...
    return {'total_revenue': total_revenue, 'total_orders': total_orders, 'total_units': total_units, 'average_order_value': average_order_value}
...
def revenue_by_dimension(dataframe, dimension):
    ...
    if missing_columns:
        raise ValueError(f'Missing required columns: {sorted(missing_columns)}')
    return dataframe.groupby(dimension, dropna=False).agg(Revenue=('Revenue', 'sum'), Orders=('Order_ID', 'nunique'), Units=('Quantity', 'sum')).reset_index().sort_values('Revenue', ascending=False).reset_index(drop=True)
