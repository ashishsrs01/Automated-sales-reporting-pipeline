...
def main() -> None:
    ...
    for month_number, month_name in enumerate(config.months, start=1):
        clean_data = generate_monthly_sales(
            year=2026,
            month=month_number,
        ...
        corrupted_data, stats = corrupt_dataset(
            clean_data,
            rng,
        )
        ...
        ground_truth[filename] = {
            ...
            "final_rows": len(corrupted_data),
            "duplicate_rows": stats.duplicate_rows,
            "missing_values": stats.missing_values,
            "text_errors": stats.text_errors,
            "date_errors": stats.date_errors,
            "invalid_quantities": stats.invalid_quantities,
            "invalid_prices": stats.invalid_prices,
        }

    metadata_path = (
        config.metadata_dir / "data_quality_ground_truth.json"
    )
