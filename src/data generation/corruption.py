...
class CorruptionStats:
    def __init__(self, duplicate_rows=0, missing_values=0, text_errors=0, date_errors=0, invalid_quantities=0, invalid_prices=0):
...
def inject_duplicates(dataframe, rng, rate=0.015):
    ...
    if count == 0:
        return (dataframe, 0)
    duplicate_rows = dataframe.sample(n=count, random_state=int(rng.integers(0, 1000000)))
...
def corrupt_dataset(dataframe, rng):
    ...
    result, stats.invalid_quantities, stats.invalid_prices = inject_invalid_numbers(result, rng)
    return (result, stats)